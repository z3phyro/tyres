...
class TranslationService {
  static getTranslationsObject = async (dictionaries: string[]): Promise<TDataNode[]> => {
    const promiseList = dictionaries.map((dict: string) =>
  ...
  static getTranslationsTable = async (index: number, dictionaries: string[]): Promise<string[][]> => {
    const translations = await TranslationService.getTranslationsObject(dictionaries);
